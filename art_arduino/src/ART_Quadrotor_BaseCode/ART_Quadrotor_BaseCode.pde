...
#include "ART_Quadrotor_BaseCode.h"
...
void loop() {
 ...
 /* if ( millis() - sonarTimer > 100 ) {
    ...
    if (abs(previousAltitude - sonarAltitude) < 10)
    {
    ...
    flag = 1;
    actualAltitude = 0.7*actualAltitude + (1-0.7)*usedAltitude;
     
   Serial.print(controlAltitude);  Serial.print("       "); Serial.print(altitudeError);Serial.print(sonarAltitude);Serial.print("      ");  Serial.print(usedAltitude); Serial.print("      ");  Serial.println(actualAltitude);   //Serial.println(altitudeI);
  }  */
  
  if ( millis() - sonarTimer > 33) {
    loopDt_slow = (millis() - sonarTimer)/1000.0;
    sonarTimer = millis();
   ...
   // Serial.print(desiredAltitude); Serial.print("      ");  Serial.print(altitudeError); Serial.print("      "); Serial.print(altitudeI); Serial.print("      "); Serial.println(controlAltitude);
  ...
  // Computer controlled mode
    ...
    if ( isLanding == true ) {
       desiredAltitude = constrain(landingAltitude - ((millis() - landingTime)/75),0,200);
         ...
         */
      desiredAltitude = constrain(map(pilotThrottle,1100,2000, 17,199),10,200);
    ...
    if ( motorsArmed == 1 ) {
      ...
      motor[0] = constrain(throttle+controlRoll-controlPitch+controlYaw+(flag*controlAltitude)/*+controlXpose+controlYpose-controlThetapose*/,1100,2000);
      motor[1] = constrain(throttle+controlRoll+controlPitch-controlYaw+(flag*controlAltitude)/*+controlXpose-controlYpose+controlThetapose*/,1100,2000);
      motor[2] = constrain(throttle-controlRoll+controlPitch+controlYaw+(flag*controlAltitude)/*-controlXpose-controlYpose-controlThetapose*/,1100,2000);
      motor[3] = constrain(throttle-controlRoll-controlPitch-controlYaw+(flag*controlAltitude)/*-controlXpose+controlYpose+controlThetapose*/,1100,2000);
...
void PIDControl() {
  ...
  altitudeI += altitudeError*loopDt_slow;
  altitudeI = constrain(altitudeI,-1000,1000);

  altitudeD = (altitudeError - altitudeErrorOld)/loopDt_slow;
