...
public:
	...
	if ((msg_lines->theta_index.size() != 0) && (msg_lines->est_rho.size() != 0) && (msg_lines->endpoints.size() != 0)) {
		...
		//float delta_theta = 1;
		//float pi = 3.1415926;
		int num_theta_entries = (2 * M_PI - delta_theta*angle_increment)/(delta_theta*pi/180) + 1;
		vector<float> theta = vector<float>(num_theta_entries);

		for(double i = -pi, counter = 0; counter < num_theta_entries; i+= delta_theta*pi/180, counter++) {
		    theta[counter] = i;
		...
		*/
		...
		if (doNextRun == true) {
			...
			for (int j=0; j < scan1->size(); j++)
			{
			    ...
			    expected_rho_change = translation_prior_mean[0]*cos(theta[scan1->at(j).theta_index]) + translation_prior_mean(1)*sin(theta[scan1->at(j).theta_index]);
			    if(abs(scan2->at(i).est_rho - (scan1->at(j).est_rho + expected_rho_change)) > 0.3)
			    {
				continue;
			    }
