...
//    float rotation_prior_mean = previous_est_rot;
	...
	for (int j=0; j < secondScan->size(); j++)
	{
	    ...
	    if(abs(firstScan->at(i).est_rho + expected_rho_change - secondScan->at(j).est_rho) > 0.3)
	    {
		continue;
	    }
	...
	for (int j=0; j < secondScan->size();j++)
	{
		...
		if(abs(firstScan->at(i).est_rho - (secondScan->at(j).est_rho + expected_rho_change)) < 0.3)
		{
